...
it('throws locally when webhook url is missing for subscribe and unsubscribe', function () {
    $http = Mockery::mock(HttpClient::class);
    $http->shouldReceive('request')->never();

    $subscription = new MaxSubscription(new MaxClient('token', $http));

    expect(fn () => $subscription->subscribe())
        ->toThrow(CouldNotSendNotification::class, 'MAX webhook URL is required for subscribe/unsubscribe.');

    expect(fn () => $subscription->unsubscribe())
        ->toThrow(CouldNotSendNotification::class, 'MAX webhook URL is required for subscribe/unsubscribe.');
});
