...
it('dispatches NotificationFailed and invokes onError when recipient resolution fails', function () {
    $notifiable = new TestNotifiable(maxUserId: null);
    $notification = new class extends Notification
    {
        ...
        public function toMax($notifiable): MaxMessage
        {
            return MaxMessage::create('Recipient required')->onError(function (array $data): void {
                $this->captured = $data;
            });
        }
    };
    ...
    try {
        ...
        expect($exception->getMessage())->toBe('No MAX recipient was provided. Set user_id/chat_id on the message or configure routeNotificationForMax().')
            ->and($notification->captured['to'])->toBe([])
            ->and($notification->captured['request'])->toBe([
                'query' => [],
                'body' => ['text' => 'Recipient required'],
            ])
            ->and($notification->captured['exception'])->toBeInstanceOf(CouldNotSendNotification::class);
    }
});

it('dispatches NotificationFailed when media preparation fails before message sending', function () {
    $notifiable = new TestNotifiable(maxUserId: 12345);
    ...
    {
        public function __construct(private readonly MaxClient $client) {}

        public function toMax($notifiable): MaxMessage
        {
            return MaxMessage::create('Broken media')
                ->withClient($this->client)
                ->toUser(12345)
                ->image('/definitely-missing-max-file.jpg');
        }
    };
    ...
    $this->dispatcher
        ...
        ->with($this->callback(function (NotificationFailed $event) use ($notifiable, $notification): bool {
            return $event->channel === 'max'
                ...
                && $event->data['request'] === [
                    'query' => ['user_id' => 12345],
                    'body' => ['text' => 'Broken media'],
                ];
        }));

    $this->channel->send($notifiable, $notification);
})->throws(CouldNotSendNotification::class, 'Invalid file path: /definitely-missing-max-file.jpg');
